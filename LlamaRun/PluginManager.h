...
public:
	...
	void BroadcastEvent(const std::string& eventName) {
		...
		//{
		//	auto plugin = pluginInspectable.as<LlamaRun::Plugin>();

		//	if (plugin.isPluginEnabled())
		//	{
		//		auto it = plugin.as<LlamaRun::Plugin>().actions.find(eventName);
		//		if (it != plugin.as<Plugin>()->actions.end()) {
		//			const std::string& methodName = it->second;
		//			PyObject_CallMethod(plugin.as<Plugin>()->instance, methodName.c_str(), nullptr);  // Call method based on method name
		//		}
		//		else {
		//			std::cerr << "Event " << eventName << " not found in plugin " << to_string(plugin.as<Plugin>()->name) << std::endl;
		//		}
		//	}
		//}
	}
